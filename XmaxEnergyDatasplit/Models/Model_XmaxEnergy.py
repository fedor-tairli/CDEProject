...
class LossBuffer:
    ...
    def is_ready(self, min_samples=10):
        return self.count >= min_samples                 # dont clip before we have enough history
...
class Loss_class():
    ...
    def __init__(self, Loss_info, **kwargs):
        ...
        self.Loss_Buffers = {key: LossBuffer(maxlen=100) for key, out_weight in zip(self.keys, self.OutWeights) if out_weight > 0}
        ...
        for i,key in enumerate(self.keys):
            if self.OutWeights[i] == 0:
                ...
                if i_buffer.is_ready():
                    ...
                    upper_bound = mean + 3*std #only check upper bound
                    i_Loss_mask = (i_Loss < upper_bound)
                    if i_Loss_mask.any():
                        i_Loss  = i_Loss[i_Loss_mask]
                
                i_Loss = i_Loss.mean()
                i_buffer.push(i_Loss)
...
class Validate_class():
    ...
    def __call__(self, model, Dataset, Loss, Extra_Loss_Info, **kwargs):
        ...
        return Loss(Preds, Truths, Loss_info)
