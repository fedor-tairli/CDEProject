...
class Model_SDP_NLRE_with_Conv(nn.Module):
    Name = 'Model_SDP_NLRE_with_Conv'
    Description = '''
    ...
    def __init__(self,in_main_channels = (3,2), N_kernels = 32, N_dense_nodes = 128, **kwargs):
        ...
        assert in_RecValues_channels == 2, 'Expecting 6 RecValues Channels'
        self.kwargs = kwargs
        dropout = kwargs['model_Dropout'] if 'model_Dropout' in kwargs else 0.2

        super(Model_SDP_NLRE_with_Conv, self).__init__()
        ...
        # Output Layer
        self.SDPTheta1 = nn.Linear(N_dense_nodes+8,N_dense_nodes)
        self.SDPTheta2 = nn.Linear(N_dense_nodes,N_dense_nodes//2)
        self.SDPTheta3 = nn.Linear(N_dense_nodes//2,1)

        self.SDPPhi1   = nn.Linear(N_dense_nodes+8,N_dense_nodes)
        self.SDPPhi2   = nn.Linear(N_dense_nodes,N_dense_nodes//2)  
        self.SDPPhi3   = nn.Linear(N_dense_nodes//2,1)

        self.OutWeights = torch.tensor([1,1])

    def forward(self,Main,Aux,Augmentation_Scale = 4, Augmentation_Function = 'BatchShuffle'):
        ...
        for aug_step in range(Augmentation_Scale):
            ...
            if aug_step ==0: # First one is the original
                ...
                if Augmentation_Function == 'BatchShuffle':
                    perm = torch.randperm(RecVals.shape[0])
                    Aug_RecVals = RecVals[perm]
                else:
                    raise NotImplementedError(f'Augmentation Function {Augmentation_Function} not implemented')
