...
class Model_SDP_NLRE_with_Conv(nn.Module):
    ...
    def __init__(self,in_main_channels = (2,2), N_kernels = 32, N_dense_nodes = 128, **kwargs):
        ...
        assert len(in_main_channels) == 2, 'Expecting two Mains: Traces and RecValues'
        ...
        # Output Layer
        self.SDPTheta1 = nn.Linear(N_dense_nodes+4,N_dense_nodes)
        self.SDPTheta2 = nn.Linear(N_dense_nodes,N_dense_nodes//2)
        self.SDPTheta3 = nn.Linear(N_dense_nodes//2,1)

        self.SDPPhi1   = nn.Linear(N_dense_nodes+4,N_dense_nodes)
        self.SDPPhi2   = nn.Linear(N_dense_nodes,N_dense_nodes//2)  
        self.SDPPhi3   = nn.Linear(N_dense_nodes//2,1)
...
class Model_SDP_NLRE_with_Conv_GaussianShift(Model_SDP_NLRE_with_Conv):
    Name = 'Model_SDP_NLRE_with_Conv_GaussianShift'
    Description = '''
    Convolutional Neural Network for SDP Reconstruction
    Uses standard Conv2d Layers in blocks with residual connections
    Reconstruction is done for triple telescopes
    ...
    def __init__(self,**kwargs):
        super(Model_SDP_NLRE_with_Conv_GaussianShift, self).__init__(**kwargs)
