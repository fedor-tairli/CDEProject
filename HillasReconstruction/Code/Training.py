...
def LoadProcessingDataset(Path_To_Data,Path_To_Proc_Data,RunNames,RecalculateDataset = False,NeedTraces = False,OptionalName = None):
    ...
    if RecalculateDataset:
        ...
        print(f'Looking for files in {Path_To_Data}/DoSpaceTrigger_Data_Batch*.pkl')
        all_files = sorted(glob.glob(Path_To_Data+'/DoSpaceTrigger_Data_Batch*.pkl'))


        print(f'Found {len(all_files)} files')
        # all_files = all_files[:1] # For testing, only use the first file

        Dataset = ProcessingDatasetContainer()
        Dataset.set_Name('Hillas_Processing_Dataset')

        # Pass the data to the ProcessingDataset

        Pass_Main (all_files,Dataset)
        print()
        Pass_Aux  (all_files,Dataset)
        print()
        Pass_Truth(all_files,Dataset)
        print()
        Pass_Rec  (all_files,Dataset)
        print()
        Pass_Graph(all_files,Dataset)
        print()
        Pass_MetaData(all_files,Dataset)
        print()
...
if __name__ == '__main__' and not TestingThings:
    ...
    # Reading the dataset    
    Path_To_Data      = os.path.abspath('../../Data/SpaceTriggerPickles/') + '/'
    Path_To_Proc_Data = os.path.abspath('../Data/SpaceTriggerPickles/') + '/'
