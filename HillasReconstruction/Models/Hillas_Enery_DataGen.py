...
def Main_Hillas_Parameters_and_Geometry_SaturationCut(Dataset,ProcessingDataset):
    '''Dataset should be a list of filenames'''
    IDsList = ()
    All_H_Amplitude       = []
    All_H_Distance        = []
    ...
    for file in Dataset:
        ...
        for i,Event in enumerate(Data):
            ...
            if not 'HillasValues' in Event:
                ...
                # print(Event.keys())
                # raise ValueError('HillasValues have not been calculated')
            TraceData = Event['PixelData']['Trace']
            # check if any value is greater than saturation threshold
            Saturation_Threshold = 10000
            if not np.any(TraceData > Saturation_Threshold):
                continue
...
def Truth_XmaxEnergy_SaturationCut(Dataset,ProcessingDataset):
    '''Will provide Xmax and Energy for each event'''
    IDsList = ()
    Gen_Xmax   = []
    Gen_Energy = []
...
def Aux_Descriptors_SaturationCut(Dataset, ProcessingDataset):
    ''' Will just provide some event descriptors for dependence inspection
    Values are : 'Event_Class', 'Primary', 'Gen_LogE', 'Gen_CosZenith', 'Gen_Xmax','Gen_Chi0', 'Gen_Rp', etc
    '''
