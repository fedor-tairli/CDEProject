...
class Model_1_2(nn.Module):
    ...
    def __init__(self,in_channels = 4,NDenseNodes = 32,GCNNodes = 16,attention_heads = 1,Dtype = torch.float32):
        ...
        '''
        self.Conv1 = GCNConv(in_channels,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv2 = GCNConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.MeanPool = global_mean_pool
        self.MaxPool  = global_max_pool
...
class Model_2_0(nn.Module):
    ...
    def __init__(self,in_channels = 4,NDenseNodes = 32,GCNNodes = 16,attention_heads = 1,Dtype = torch.float32):
        ...
        Added Station Data
        Trained with All nodes Connected
        '''
        self.Conv1 = GATConv(in_channels,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv2 = GATConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.MeanPool = global_mean_pool
        self.MaxPool  = global_max_pool
    ...
    def forward(self, X, Edge_index, Edge_weight,Batching):
        ...
        DeltaChi = X[Edge_index[0],0]-X[Edge_index[1],0] # Usually upstream is larger  so use I-F order
        DeltaCen = X[Edge_index[1],2]-X[Edge_index[0],2] # Usually Upstream is smaller so use F-I order
        
        # X = X[:,[1,3]]
        Edge_Features = torch.stack([DeltaChi,DeltaCen],dim=1)

        # Scan
        out = self.ConvActivation(self.Conv1(X,Edge_index,Edge_Features))
        out = self.ConvActivation(self.Conv2(out,Edge_index,Edge_Features))
        ...
        Chi0 = self.DenseActivation(self.Chi0Dense2(Chi0))
        Chi0 = self.Chi0Dense3(Chi0)
        Chi0 = self.AngleActivation(Chi0)
...
class Model_2_2(nn.Module):
    ...
    def __init__(self,in_channels = 4,NDenseNodes = 32,GCNNodes = 16,attention_heads = 1,Dtype = torch.float32):
        ...
        And Swwitch to GATConv
        Add Construction of edge features to be fed into the GATConv
            For X_Features     [Chi_i,C = Edge_index[1]>=Edge_index[0]
        # Edge_iharge,Time,Duration] [0,1,2,3]
            For Edge_Features  [DeltaChi,DeltaT] [0,2]
        Added Station Data
        Trained with Standard Nodes connections
        ...
        self.Conv1 = GATConv(in_channels,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv2 = GATConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv3 = GATConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv4 = GATConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.Conv5 = GATConv(GCNNodes,GCNNodes,heads = attention_heads,dtype=Dtype)
        self.MeanPool = global_mean_pool
        self.MaxPool  = global_max_pool
    ...
    def forward(self, X, Edge_index, Edge_weight,Batching):
        ...
        out = self.ConvActivation(self.Conv1(X,Edge_index,Edge_Features))
        out = self.ConvActivation(self.Conv2(out,Edge_index,Edge_Features))
        out = self.ConvActivation(self.Conv3(out,Edge_index,Edge_Features))
        out = self.ConvActivation(self.Conv4(out,Edge_index,Edge_Features))
        out = self.ConvActivation(self.Conv5(out,Edge_index,Edge_Features))
