...
def IterateInBatches(Dataset,BatchSize = 16):
    ...
    '''
    if Dataset.HasStationFeatures:
        counter = 0
        for EvIndex,EvFeatures,EvEdges,EvEdgesWeights,EvTruth,EvStation in Dataset:
            if EvEdges.shape[1] == 0: continue
            if counter == 0:
                Indices      = torch.tensor([EvIndex])
                Features     = torch.cat((EvFeatures,EvStation.unsqueeze(0)),dim=0)
                Batching     = torch.ones(len(EvFeatures)+1,dtype=torch.int64)*counter
                StationEdges = torch.arange(len(EvFeatures),dtype=torch.int32).unsqueeze(0)
                StationEdges = torch.cat((StationEdges,torch.ones(len(EvFeatures),dtype=torch.int32).unsqueeze(0)*(len(EvFeatures))),dim=0) # Connect the Station to all the nodes
                Edges        = torch.cat((EvEdges,StationEdges),dim=1)
                # # All nodes connected
                # n = len(EvFeatures)+1
                # Edges        = torch.stack((torch.arange(n).repeat(n),torch.arange(n).unsqueeze(1).repeat(1,n).view(-1)))
                EdgesWeights = torch.cat((EvEdgesWeights,torch.ones(len(EvFeatures))),dim=0)
                Truth        = EvTruth.unsqueeze(0)
            else:
                Indices      = torch.cat((Indices,torch.tensor([EvIndex])))
                Batching     = torch.cat((Batching,torch.ones(len(EvFeatures)+1,dtype=torch.int64)*counter))
                StationEdges = torch.arange(len(EvFeatures),dtype=torch.int32).unsqueeze(0)
                StationEdges = torch.cat((StationEdges,torch.ones(len(EvFeatures),dtype=torch.int32).unsqueeze(0)*(len(EvFeatures))),dim=0) # Connect the Station to all the nodes
                EvEdges      = torch.cat((EvEdges,StationEdges),dim=1)
                # # All nodes connected
                # n = len(EvFeatures)+1
                # EvEdges      = torch.stack((torch.arange(n).repeat(n),torch.arange(n).unsqueeze(1).repeat(1,n).view(-1)))
                EvEdges      = EvEdges+ len(Features)                                                        # Need to adjust the edges to refer to the appropriate nodes in the batch
                Features     = torch.cat((Features,EvFeatures,EvStation.unsqueeze(0)),dim = 0)
                Edges        = torch.cat((Edges,EvEdges),dim=1)
                EdgesWeights = torch.cat((EdgesWeights,EvEdgesWeights,torch.ones(len(EvFeatures))),dim=0)
...
def Train(model,Dataset,optimiser,scheduler,Loss,Validation,Tracker,\
    ...
    for i in range(Epochs):
        ...
        epoch_loss_Chi0 = epoch_loss_Chi0/batchN
        epoch_loss_Rp   = epoch_loss_Rp/batchN

        # Validation and Early stopping
        val_loss,val_loss_Chi0,val_loss_Rp = Validation(model,Dataset,Loss,device,normStateOut)
