...
class Tracker():
    ...
    def MakeLog(self,PathToLogFolder,ModelName):
        ...
        # Produce the log
        LogName = PathToLogFolder+ModelName+now().strftime("%Y-%m-%d_%A_%H-%M")+'_Log.txt'
...
def PlotOnEpoch(Dataset,Model,Epoch,plotSavePath,device):
    ...
    with torch.no_grad():
        for _,BatchMains,BatchAux,BatchTruth,_ in Dataset:
            ...
            PredList.append(predictions.cpu().detach())
...
def Train(model,Dataset,optimiser,scheduler,Loss,Validation,Metric,Tracker,\
    ...
    for i in range(Epochs):
        ...
        for _, BatchMains, BatchAux, BatchTruth, _ in Dataset: # Dataset Event index is not used
            ...
            BatchMains     = [Main.to(device) for Main in BatchMains]  # Multiple Mains are possible
            BatchAux       = BatchAux   .to(device) # Tensor containing the extra event data - eg. station data
