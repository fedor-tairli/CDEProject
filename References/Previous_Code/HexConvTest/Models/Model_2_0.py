...
#           Iteration now happens on a filename basis        #
##############################################################
...
class Task_Block(nn.Module):
    def __init__(self, in_channels, out_channels): 
        ...
        self.ResBlock3 = ResidualBlock(in_channels=in_channels, out_channels=in_channels, kernel_size=1, stride=1)

        # Global AvPool
        self.GlobAvPool = nn.AdaptiveAvgPool2d(1)
        # 2 FC layer
        self.FC1 = nn.Linear(in_channels*3, 128)
    ...
    def forward(self, x):
        ...
        out2 = self.ResBlock2(x)
        out3 = self.ResBlock3(x)
        # Concatentate the 3 outputs
        out = torch.cat((out1,out2,out3),dim=1)
        del out1,out2,out3
        out = self.GlobAvPool(out)
        out = out.reshape(-1, self.in_channels *3)
...
class Model_2_0(nn.Module):
    ...
    def __init__(self):
        ...
        self.Name = 'Model_2_0'
        self.Description ='''
        Thinking that the lack of freedom in the task blocks is the problem
        - Use Residual blocks in parallel rather than in series
        - Increase the Size of FC layers at the end for more freedom
        Thats all not correct, it seems, but model does perform fine in other tasks

        Thinking that i might adjust the Loss function for Xmax to be scaled by True Xmax
        - that way the outer ranges are more important as there are less of them, balance
        -> Dint help, Nothing changed, might try Regularisation
        '''
...
def Loss(y_pred, y_true,coeffs):
    ...
    X_loss = coeffs[3]*torch.mean(torch.abs(y_true[3])*(y_pred[3] - y_true[3])**2) # multiply by abs value of Xmax (remember centered about zero), so outer ranges are more important
    # X_loss = coeffs[3]*torch.mean((y_pred[3] - y_true[3])**2)  # Original

    T_Loss = E_loss + C_loss + A_loss+ X_loss
    
    return T_Loss, E_loss, C_loss, A_loss, X_loss

def Loss_2(y_pred, y_true,coeffs):
