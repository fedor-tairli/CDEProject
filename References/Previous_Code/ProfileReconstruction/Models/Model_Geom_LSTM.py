...
# def Loss(Pred,Truth,keys=['Chi0','Rp','T0','SDPTheta','SDPPhi'],ReturnTensor = True):
...
# def metric(model,Dataset,device,keys = ['Chi0','Rp','T0','SDPTheta','SDPPhi'],BatchSize = 256):
#     '''
#     Takes model, Dataset, Loss Function, device, keys
#     Dataset is defined as ProcessingDatasetContainer in the Dataset2.py
#     keys are to be used in the loss function
...
# Define the model
class Model_Geom_LSTM_0(nn.Module):
    Name = 'Model_Geom_LSTM_0'
    Description = '''
    Simple LSTM Model for Geometry Reconstruction
    '''
    def __init__(self, in_main_channels=1, in_aux_channels_aux=1, N_LSTM_nodes = 32, N_LSTM_layers = 3, N_dense_nodes=128, dtype = torch.float32,**kwargs):
        ...
        self.kwargs = kwargs
        # Input is 20x22 grid
        super(Model_Geom_LSTM_0,self).__init__()
        ...
        # Output Layers
        self.Chi1 = nn.Linear(N_dense_nodes,N_dense_nodes)
        self.Chi2 = nn.Linear(N_dense_nodes,N_dense_nodes)
        self.Chi3 = nn.Linear(N_dense_nodes,1)
    ...
    def forward(self, Mains, AuxData):
        ...
        # Chi
        Chi = self.DenseActivation(self.Chi1(out))
        Chi = self.DenseActivation(self.Chi2(Chi))
        Chi = self.AngleActivation(self.Chi3(Chi))
        ...
        return torch.cat([Chi,Rp,T0],dim=1)
    



# Define the model
class Model_Geom_LSTM_1(nn.Module):
    Name = 'Model_Geom_LSTM_1'
    Description = '''
    Simple LSTM Model for Geometry Reconstruction
    '''
    def __init__(self, in_main_channels=1, in_aux_channels_aux=1, N_LSTM_nodes = 32, N_LSTM_layers = 3, N_dense_nodes=128, dtype = torch.float32,**kwargs):
        ...
        self.kwargs = kwargs
        # Input is 20x22 grid
        super(Model_Geom_LSTM_1,self).__init__()
        # in_Channels should be a tuple of the number of channels for each Main, Only have 1 main here
        in_channels = in_main_channels[0]
    ...
    def forward(self, Mains, AuxData):
        ...
        out = self.LSTM_input(Main)[0]
        out = self.LSTM_output(out)[0]
        # Aux Data Has the last position of the trace, use it to get the last output
        out = out[torch.arange(out.shape[0]),AuxData[:,-1].long(),:]
...
# Kinda weird things, but I will keep them for now


# Define the model
class Model_Geom_LSTM_D(nn.Module):
    Name = 'Model_Geom_LSTM_D'
    Description = '''
    Simple LSTM Model for Geometry Reconstruction
    '''
    def __init__(self, in_main_channels=1, in_aux_channels_aux=1, N_LSTM_nodes = 32, N_LSTM_layers = 3, N_dense_nodes=128, dtype = torch.float32,**kwargs):
        ...
        self.kwargs = kwargs
        # Input is 20x22 grid
        super(Model_Geom_LSTM_D,self).__init__()
        # in_Channels should be a tuple of the number of channels for each Main, Only have 1 main here
        in_channels = in_main_channels[0]
        ...
        self.LSTM_input  = nn.Linear(1000*3,N_LSTM_nodes)
        self.LSTM_output = nn.Linear(N_LSTM_nodes,N_LSTM_nodes)
    ...
    def forward(self, Mains, AuxData):
        device = self.LSTM_input.weight.device
        assert len(Mains) == 1, 'Only 1 Main is allowed in this model'
        Mains = Mains[0]
        Mains = Mains.to(device)
        # LSTM

        Mains = Mains.view(Mains.shape[0],-1)

        out = self.LSTM_input(Mains)
        out = self.LSTM_output(out)
        # out = out[:,-1,:]
...
# Define the model
class Model_Geom_LSTM_SDP_LSTM(nn.Module):
    Name = 'Model_Geom_LSTM_SDP_LSTM'
    Description = '''
    Simple LSTM Model for Geometry Reconstruction
    '''
    def __init__(self, in_main_channels=1, in_aux_channels_aux=1, N_LSTM_nodes = 32, N_LSTM_layers = 3, N_dense_nodes=128, dtype = torch.float32,**kwargs):
        ...
        self.kwargs = kwargs
        # Input is 20x22 grid
        super(Model_Geom_LSTM_SDP_LSTM,self).__init__()
        # in_Channels should be a tuple of the number of channels for each Main, Only have 1 main here
        in_channels = in_main_channels[0]
    ...
    def forward(self, Mains, AuxData):
        ...
        return torch.cat([Chi,Rp,T0,Theta,Phi],dim=1)
