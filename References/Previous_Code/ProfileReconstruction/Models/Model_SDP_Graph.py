...
# Define the model
class Model_SDP_Graph_C(nn.Module):
    Name = 'Model_SDP_Graph_C'
    Description = '''
    ...
    def __init__(self, in_node_channels=5, in_edge_channels = 2, N_Graph_Heads = 16, N_kernels = 16, kernel_size = 10,N_dense_nodes=128, dtype = torch.float32,**kwargs):
        ...
        self.kwargs = kwargs
        # Input is 20x22 grid
        super(Model_SDP_Graph_C,self).__init__()
        ...
        # Conv1d Layers ( there would be some math that makes the input to this a padded 1000xN_Graph_Heads tensor)
        self.Conv1 = nn.Conv1d(2*in_node_channels*N_Graph_Heads,N_kernels, kernel_size=kernel_size, stride=2, padding=1)               # out -> 497xNch
        self.Conv2 = nn.Conv1d(N_kernels,N_kernels                       , kernel_size=kernel_size, stride=2, padding=1) # out -> 
        self.Conv3 = nn.Conv1d(N_kernels,N_kernels                       , kernel_size=kernel_size, stride=2, padding=1) # out -> 
        ...
        # Dense Layers
        self.Dense1 = nn.Linear(352 ,N_dense_nodes) # TODO: Fix This Hard Code 352
    ...
    def forward(self, Graphs, AuxData):
        ...
        Max_out,_ = max_pool_x(cluster = Timing, x = out, batch = Batching, size = 1000)
        
        Avg_out,_ = avg_pool_x(cluster = Timing, x = out, batch = Batching, size = 1000)
        # Reshape for Conv1d
        out = torch.cat([Max_out.view(-1,out.shape[1],1000),Avg_out.view(-1,out.shape[1],1000)],dim=1)
