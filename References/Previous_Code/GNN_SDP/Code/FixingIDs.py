...
for runN,run in enumerate(['Run010','Run030','Run080','Run090']):
    ...
    for i in range(Dataset._otherData.shape[0]):
        primary = Dataset._otherData[i,7].item()
        Energy  = Dataset._otherData[i,8].item()
        id      = Dataset._otherData[i,6]
