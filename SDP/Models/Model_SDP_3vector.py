...
def metric(model,Dataset,device,keys=['X','Y','Z'],BatchSize = 256):
    ...
    dot_product = torch.sum(Preds * Truths, dim=-1)
    norm_Preds  = torch.norm(Preds , dim=-1)
    norm_Truths = torch.norm(Truths, dim=-1)
    cos_angle = dot_product / (norm_Preds * norm_Truths)
    cos_angle = torch.clamp(cos_angle, -1.0, 1.0)
    angle = torch.acos(cos_angle)
    
    # Compute the 68% percentrile of the deviation
    metrics['X'] = torch.quantile(angle, 0.68).item()
    metrics['Y'] = metrics['X']
    metrics['Z'] = metrics['X']
...
class Model_SDP_3vector(nn.Module):
    ...
    def __init__(self,in_main_channels = (1,), N_kernels = 32, N_dense_nodes = 128, **kwargs):
        ...
        self.Dense5   = nn.Linear(N_dense_nodes,N_dense_nodes//2)  
        self.Dense6   = nn.Linear(N_dense_nodes//2,3)

        self.OutWeights = torch.tensor([1,1,1]) # For legacy reasons i want to keep this
    ...
    def forward(self,Main,Aux):
        ...
        Main = self.Angle_Activation(self.Dense6(Main)) # Using Tanh to normalise the output to [-1,1] range

        # Convert 3-vector to a unit vector
        Main = Main / torch.norm(Main, dim=-1, keepdim=True)

        return Main  # Apply the weights to the output
