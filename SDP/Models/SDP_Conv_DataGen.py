...
def Truth_SDP_via3vector(Dataset,ProcessingDataset):
    ...
    # Convert to 3-vector
    Gen_XYZ = spherical_to_cartesian(Gen_SDPTheta, Gen_SDPPhi)
    Rec_XYZ = spherical_to_cartesian(Rec_SDPTheta, Rec_SDPPhi)

    if ProcessingDataset is None:
        return Gen_XYZ, Rec_XYZ
    ProcessingDataset._Truth = Gen_XYZ
    ProcessingDataset._Rec   = Rec_XYZ
