...
main (int argc, char **argv)
{
  ...
  for (const string& file : filenames) {
    ...
    for (unsigned int iEvent = 0; iEvent < ntotThisFile; iEvent++){
      ...
      string EventID = theRecEvent->GetEventId();
      ...
      { // Scope to get the HEAT Variables
        ...
        double Gen_SDPPhi      = HE_GenGeometry.GetSDPPhi()*180/kPi;
        double Gen_SDPTheta    = HE_GenGeometry.GetSDPTheta()*180/kPi;
        ...
        // Gen_CherenkovFraction = HE_TelData.GetGenApertureLight().GetCherenkovFraction();
        

        cout << "Event " << EventID << ": "
             << "Gen_SDPPhi: " << std::setw(6) << std::fixed << std::setprecision(2) << Gen_SDPPhi << ", "
             << "Gen_SDPTheta: " << std::setw(6) << std::fixed << std::setprecision(2) << Gen_SDPTheta 
             << endl;
      }
